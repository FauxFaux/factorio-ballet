import icons0Url from '../assets/dataset/bobang/icons-0.avif';
import icons0Data from '../assets/dataset/bobang/icons-0.json';
import icons1Url from '../assets/dataset/bobang/icons-1.avif';
import icons1Data from '../assets/dataset/bobang/icons-1.json';
import icons2Url from '../assets/dataset/bobang/icons-2.avif';
import icons2Data from '../assets/dataset/bobang/icons-2.json';
import icons3Url from '../assets/dataset/bobang/icons-3.avif';
import icons3Data from '../assets/dataset/bobang/icons-3.json';
import iconsUiUrl from '../assets/dataset/bobang/icons-ui.avif';
import iconsUiData from '../assets/dataset/bobang/icons-ui.json';

import './preload-icons.ts';

type Icon = [url: string, x: number, y: number, sheetSize: number];
type IconData = Record<string, [number, number]>;

function iconsFromSheet(url: string, data: unknown, sheetSize: number): Record<string, Icon> {
  return Object.fromEntries(
    Object.entries(data as IconData).map(([key, [x, y]]) => [key, [url, x, y, sheetSize]]),
  );
}

export const icons: Record<string, Icon> = {
  ...iconsFromSheet(icons0Url, icons0Data, 864),
  ...iconsFromSheet(icons1Url, icons1Data, 864),
  ...iconsFromSheet(icons2Url, icons2Data, 864),
  ...iconsFromSheet(icons3Url, icons3Data, 864),
  ...iconsFromSheet(iconsUiUrl, iconsUiData, 480),
};
