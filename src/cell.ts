 ...
 * `docs/guides/FACTORIO.md` for what a cell is for; this file is only its shape and the set arithmetic
 * over it.
 *
...
function simplestFirst(data: StaticData, ids: ResourceId[]): ResourceId[] {
  return ids.toSorted(
    (a, b) =>
      ...
      resourceName(data, a).localeCompare(resourceName(data, b)),
  );
}
