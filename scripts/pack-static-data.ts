    ...
    machines: Object.fromEntries(
      Object.entries(data.machines).map(([id, machine]) => [
        id,
        {
          ...
          s: machine.speed,
          z: machine.size,
          f: machine.fluidBoxes?.map((box) => ({
            p: box.productionType,
            c: box.connections.map((connection) => ({
              p: [connection.position.x, connection.position.y],
              d: connection.direction,
              f: connection.flowDirection,
            })),
          })),
