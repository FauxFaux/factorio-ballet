...
**Synthetic recipes** (`Recipe.synthetic`, `scripts/synthetic.ts`) are conversions the game has no
...
need the cheapest — which is why the two agree on ids. Rocket launches are still complexity-only.
