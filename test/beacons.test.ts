...
import { type Cell, entryEffects, entryRun, parseModules } from '../src/cell.ts';
import {
  type Chosen,
  chosenBeacon,
  defaultBeacon,
...
import { chosenModule, type ChosenModules, SPEED_CATEGORY } from '../src/data/modules.ts';
...
const circuits = defaultDataset.data.recipes['electronic-circuit'];
/** Two module slots, which is the machine the worked example in `docs/beacons.wiki` terms uses. */
const two = defaultDataset.data.machines['assembling-machine-2'];
const three = defaultDataset.data.machines['assembling-machine-3'];
const character = defaultDataset.data.machines['character'];
...
const VANILLA = defaultDataset.data.beacons['beacon'];
...
describe('the ingested beacons', () => {
  it('is the three the pack has, with their slots and their transmission', () => {
    expect(Object.keys(defaultDataset.data.beacons)).toEqual([
      'beacon',
      'bob-beacon-2',
      'bob-beacon-3',
    ]);
    expect(defaultDataset.data.beacons['beacon']).toEqual({
      human: 'Beacon',
      item: 'beacon',
      moduleSlots: 2,
    ...
    expect(defaultDataset.data.beacons['bob-beacon-3'].moduleSlots).toBe(6);
  });
  ...
  it('are cheapest first, with an item in the data to draw each of them with', () => {
    ...
    for (const { id, beacon, complexity } of beaconTiers) {
      expect(defaultDataset.data.resources[`item:${beacon.item}`]?.human, id).toBeTruthy();
...
describe('the chosen beacon', () => {
  ...
  it('follows the slider where the header picked nothing', () => {
    ...
    expect(chosenBeacon(defaultDataset, undefined, 1)).toBe(
      defaultDataset.data.beacons['bob-beacon-3'],
    );
  });
});
...
describe('moduleBoost', () => {
  ...
  /* The worked example: a machine which holds two, with three two-slot beacons. */
  it('fills every requested beacon with speed modules', () => {
    const eight = boost(two, 2, 2, 3);
    expect(eight).toMatchObject({ wanted: 2, inMachine: 2, inBeacons: 6, beacons: 3 });
  ...
  it('uses the whole requested beacon count for the penalty', () => {
    const six = boost(two, 2, 2, 3);
    expect(six).toMatchObject({ inMachine: 2, inBeacons: 6, beacons: 3 });
    expect(six.transmission).toBeCloseTo(0.866);
  });
  ...
  it('leaves the slots the loadout already took', () => {
    ...
    expect(boost(three, 1, 1, 2)).toMatchObject({ inMachine: 1, inBeacons: 4, beacons: 2 });
  });
  ...
  it('beacons a machine which will not take the module itself', () => {
    ...
    expect(boost(picky, undefined, 3, 2)).toMatchObject({ inMachine: 0, inBeacons: 4, beacons: 2 });
...
describe('a cell row with beacons', () => {
  ...
  const entry = { recipe: 'iron-gear-wheel', machine: 'assembling-machine-2', beacons: 3 };
  ...
  it('scales what the rest of the cell has to keep up with', () => {
    ...
    const plates = (c: Cell) =>
      solveCell(defaultDataset, defaultDataset.data, c, 0, kit({ speed: SPEED_3 })).counts[1]!;
    ...
    expect(
      solveCell(defaultDataset, defaultDataset.data, cell(3), 0, noChoice(defaultDataset))
        .counts[1],
    ).toBeCloseTo(
      solveCell(defaultDataset, defaultDataset.data, cell(0), 0, noChoice(defaultDataset))
        .counts[1]!,
    );
  });
});
...
describe('moduleLayout', () => {
  ...
  it('fills the slots with productivity where the recipe pays for it', () => {
    ...
    const auto = entryRun(defaultDataset, defaultDataset.data, row, gears, row.machine, kit(both));
  ...
  it('grows a farm with the agricultural modules', () => {
    ...
    const grown = defaultDataset.data.recipes['angels-desert-5'];
    const run = entryRun(
      defaultDataset,
      defaultDataset.data,
      farm,
  ...
  it('says what could reach the machine at all, which is what a row draws a box for', () => {
    ...
    const pump = defaultDataset.data.recipes['synthetic:pumping-water'];
    expect(
      entryRun(
        defaultDataset,
        defaultDataset.data,
    ...
    expect(
      entryRun(defaultDataset, defaultDataset.data, row, gears, row.machine, kit(both)).layout
        .reaches,
    ).toEqual({
      speed: true,
      productivity: true,
