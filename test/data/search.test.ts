...
describe('resolveResources', () => {
  it('takes an exact resource id alone', () => {
    expect([...resolveResources(defaultDataset.data, 'item:iron-plate')]).toEqual([
      'item:iron-plate',
    ]);
  });

  it('takes an exact internal name over the substring matches', () => {
    expect([...resolveResources(defaultDataset.data, 'water')]).toEqual(['fluid:water']);
  });

  it('takes an exact human name', () => {
    expect([...resolveResources(defaultDataset.data, 'Iron plate')]).toEqual(['item:iron-plate']);
  });

  it('falls back to substring matches', () => {
    const found = resolveResources(defaultDataset.data, 'plate');
    expect(found.has('item:iron-plate')).toBe(true);
    expect(found.size).toBeGreaterThan(1);
  });

  it('finds nothing for nothing', () => {
    expect(resolveResources(defaultDataset.data, '')).toEqual(new Set());
    expect(resolveResources(defaultDataset.data, 'definitely-not-a-resource')).toEqual(new Set());
  });

  it('reads an @-query off the cell being worked on', () => {
    expect(resolveResources(defaultDataset.data, '@in', scope)).toEqual(scope.in);
    expect(resolveResources(defaultDataset.data, '@out', scope)).toEqual(scope.out);
    expect(resolveResources(defaultDataset.data, '@edge', scope)).toEqual(
      new Set(['item:angels-ore1-crushed', 'item:iron-gear-wheel']),
    );
  });

  it('finds nothing for an @-query with no cell, or no such query', () => {
    expect(resolveResources(defaultDataset.data, '@in')).toEqual(new Set());
    expect(resolveResources(defaultDataset.data, '@nonsense', scope)).toEqual(new Set());
  });
});
...
describe('parseSearch', () => {
  it('recognises makes: and uses: prefixes', () => {
    expect(parseSearch(defaultDataset.data, 'makes:item:iron-plate uses:water')).toEqual([
  ...
  it('treats anything else as text', () => {
    expect(parseSearch(defaultDataset.data, '  iron  Plate ')).toEqual([
      { kind: 'text', text: 'iron' },
...
describe('searchRecipes', () => {
  it('answers an @-query about the cell it was given', () => {
    const found = searchRecipes(defaultDataset.data, 'makes:@in', 0, scope);
    ...
    expect(searchRecipes(defaultDataset.data, 'makes:@in', 0)).toEqual([]);
  });

  it('finds nothing without a search', () => {
    expect(searchRecipes(defaultDataset.data, '   ', 0)).toEqual([]);
  });

  it('finds the recipes producing a resource', () => {
    const found = searchRecipes(defaultDataset.data, 'makes:item:iron-plate', 0);
    expect(found.length).toBeGreaterThan(0);
    for (const { recipe } of found) {
  ...
  it('finds the recipes consuming a resource', () => {
    const found = searchRecipes(defaultDataset.data, 'uses:item:iron-plate', 0);
    expect(found.length).toBeGreaterThan(0);
    for (const { recipe } of found) {
  ...
  it('matches recipes by name and by id', () => {
    expect(searchRecipes(defaultDataset.data, 'iron-plate', 0).map((m) => m.id)).toContain(
      'iron-plate',
    );
    expect(searchRecipes(defaultDataset.data, 'Iron plate', 0).map((m) => m.id)).toContain(
      'iron-plate',
  ...
  it('requires every term to match', () => {
    const both = searchRecipes(
      defaultDataset.data,
      'makes:item:iron-plate uses:item:iron-plate',
      0,
    );
    ...
    expect(both.length).toBeLessThan(
      searchRecipes(defaultDataset.data, 'makes:item:iron-plate', 0).length,
    );
  });

  it('puts what you can nearly build first, not what is simplest', () => {
    const at = (progress: number) =>
      searchRecipes(defaultDataset.data, 'uses:item:iron-plate', progress);
...
describe('searchMatches', () => {
  it('interleaves free-text resource and recipe matches by relevance', () => {
    const found = searchMatches(defaultDataset.data, 'iron-plate', 0);
  ...
  it('keeps directed matches recipe-only', () => {
    expect(searchMatches(defaultDataset.data, 'makes:item:iron-plate', 0)).not.toContainEqual(
