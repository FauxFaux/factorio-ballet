...
describe('resolveResources', () => {
  it('takes an exact resource id alone', () => {
    expect([...resolveResources(staticData, 'item:iron-plate')]).toEqual(['item:iron-plate']);
  });

  it('takes an exact internal name over the substring matches', () => {
    expect([...resolveResources(staticData, 'water')]).toEqual(['fluid:water']);
  });

  it('takes an exact human name', () => {
    expect([...resolveResources(staticData, 'Iron plate')]).toEqual(['item:iron-plate']);
  });

  it('falls back to substring matches', () => {
    const found = resolveResources(staticData, 'plate');
    expect(found.has('item:iron-plate')).toBe(true);
    expect(found.size).toBeGreaterThan(1);
  });

  it('finds nothing for nothing', () => {
    expect(resolveResources(staticData, '')).toEqual(new Set());
    expect(resolveResources(staticData, 'definitely-not-a-resource')).toEqual(new Set());
  });

  it('reads an @-query off the cell being worked on', () => {
    expect(resolveResources(staticData, '@in', scope)).toEqual(scope.in);
    expect(resolveResources(staticData, '@out', scope)).toEqual(scope.out);
    expect(resolveResources(staticData, '@edge', scope)).toEqual(
      new Set(['item:angels-ore1-crushed', 'item:iron-gear-wheel']),
    );
  });

  it('finds nothing for an @-query with no cell, or no such query', () => {
    expect(resolveResources(staticData, '@in')).toEqual(new Set());
    expect(resolveResources(staticData, '@nonsense', scope)).toEqual(new Set());
  });
});
...
describe('parseSearch', () => {
  it('recognises makes: and uses: prefixes', () => {
    expect(parseSearch(staticData, 'makes:item:iron-plate uses:water')).toEqual([
  ...
  it('treats anything else as text', () => {
    expect(parseSearch(staticData, '  iron  Plate ')).toEqual([
      { kind: 'text', text: 'iron' },
...
describe('searchRecipes', () => {
  it('answers an @-query about the cell it was given', () => {
    const found = searchRecipes(staticData, 'makes:@in', 0, scope);
    ...
    expect(searchRecipes(staticData, 'makes:@in', 0)).toEqual([]);
  });

  it('finds nothing without a search', () => {
    expect(searchRecipes(staticData, '   ', 0)).toEqual([]);
  });

  it('finds the recipes producing a resource', () => {
    const found = searchRecipes(staticData, 'makes:item:iron-plate', 0);
    expect(found.length).toBeGreaterThan(0);
    for (const { recipe } of found) {
  ...
  it('finds the recipes consuming a resource', () => {
    const found = searchRecipes(staticData, 'uses:item:iron-plate', 0);
    expect(found.length).toBeGreaterThan(0);
    for (const { recipe } of found) {
  ...
  it('matches recipes by name and by id', () => {
    expect(searchRecipes(staticData, 'iron-plate', 0).map((m) => m.id)).toContain('iron-plate');
    expect(searchRecipes(staticData, 'Iron plate', 0).map((m) => m.id)).toContain('iron-plate');
  });

  it('requires every term to match', () => {
    const both = searchRecipes(staticData, 'makes:item:iron-plate uses:item:iron-plate', 0);
    for (const { recipe } of both) {
      expect(recipe.products.some((p) => p.resource === 'item:iron-plate')).toBe(true);
      expect(recipe.ingredients.some((i) => i.resource === 'item:iron-plate')).toBe(true);
    }
    expect(both.length).toBeLessThan(searchRecipes(staticData, 'makes:item:iron-plate', 0).length);
  });

  it('puts what you can nearly build first, not what is simplest', () => {
    const at = (progress: number) => searchRecipes(staticData, 'uses:item:iron-plate', progress);
...
describe('searchMatches', () => {
  it('interleaves free-text resource and recipe matches by relevance', () => {
    const found = searchMatches(staticData, 'iron-plate', 0);
  ...
  it('keeps directed matches recipe-only', () => {
    expect(searchMatches(staticData, 'makes:item:iron-plate', 0)).not.toContainEqual(
