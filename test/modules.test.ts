...
const assembler = defaultDataset.data.machines['assembling-machine-3'];
const farm = defaultDataset.data.machines['angels-crop-farm'];
...
const drill = defaultDataset.data.machines['bob-mining-drill-4'];
...
describe('the ingested modules', () => {
  ...
  it('leaves the name and the icon on the item, which is the same id', () => {
    for (const id of Object.keys(defaultDataset.data.modules)) {
      expect(defaultDataset.data.resources[`item:${id}`]?.human).toBeTruthy();
    }
  });
});
...
describe('modulesFor', () => {
  ...
  it('offers speed and productivity where the recipe allows productivity', () => {
    const found = ids(defaultDataset.data, assembler, gears);
  ...
  it('drops productivity modules on a recipe which does not allow it', () => {
    const found = ids(defaultDataset.data, assembler, circuits);
    expect(found).toContain('speed-module-3');
  ...
  it('offers a module only where its category is allowed', () => {
    ...
    expect(ids(defaultDataset.data, assembler, gears)).not.toContain('angels-bio-yield-module');
    expect(ids(defaultDataset.data, farm, garden)).toContain('angels-bio-yield-module');
  });

  it('offers nothing to a machine with no slots', () => {
    expect(ids(defaultDataset.data, character, gears)).toEqual([]);
  });
  ...
  it('reads a missing allowed_effects as no restriction', () => {
    ...
    expect(allowsEffect(defaultDataset.data.machines['angels-barreling-pump'], 'speed')).toBe(true);
    expect(
      allowsEffect(defaultDataset.data.machines['angels-barreling-pump'], 'productivity'),
    ).toBe(false);
  });
});
...
describe('moduleEffects', () => {
  ...
  it('gives no productivity on a recipe which does not allow it, and still charges the speed', () => {
    ...
    expect(moduleEffects(defaultDataset.data, assembler, fill, circuits)).toEqual({
      ...moduleEffects(defaultDataset.data, assembler, fill, gears),
      productivity: 1,
    });
  });
  ...
  it('takes only as many modules as the machine has slots', () => {
    ...
    expect(
      moduleEffects(defaultDataset.data, assembler, { 'speed-module-3': 5 }, gears).speed,
    ).toBeCloseTo(2.2);
  });
  ...
  it('will not slow a machine below a fifth of its speed', () => {
    ...
    const effects = moduleEffects(
      ...
      defaultDataset.data.recipes['synthetic:mining-coal'],
    );
...
describe('moduleFor', () => {
  ...
  const desert = defaultDataset.data.machines['angels-desert-farm'];
